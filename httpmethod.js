        ...
        req.on('end', async () => {
            ...
            //INSERT the data
            {
            ...
            else if (method === 'PUT')
             {
                ...
                const result = await db.collection(colllectionName).updateOne({_id: new ObjectId(id) },
                    { $set: update }
                );
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify(result));
                ...
                const {id}=data;
                const result = await db.collection(colllectionName).deleteOne({_id: new ObjectId(id) });
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify(result));
            }
        })
